...
public class CarSimulating {
	public static void main(String[] args) {
		...
		car.getYear(num1); // car 클래스(생성자)에 저장
		
		System.out.println("Car's maker: (e.g. 'Honda')");
		String num2 = input.next(); // 제조회사를 입력 받아
		car.getMaker(num2); // car 클래스(생성자)에 저장
		
		System.out.println("Car's maximum speed: (e.g. 85.0)");
		double num3 = input.nextDouble(); // 최대 속도를 입력 받아
		car.getSpeed(num3); // car 클래스(생성자)에 저장
		
		System.out.println("차의 제작연도: " + car.year); // 생성자에 저장한 제작연도 리턴
		System.out.println("차의 제조회사: " + car.maker); // 생성자에 저장한 제조회사 리턴
		System.out.println("차의 최대속도: " + car.speed); // 생성자에 저장한 최대 속도 리턴
